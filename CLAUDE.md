...
- `src/core/orchestrator.py` — the heart. Conversation loop (`process_input`, `process_input_streaming`, `process_audio`), memory retrieval/storage, the interactive `run_text_mode` / `run_voice_mode` loops, slash-command handling, and the overlapping streaming-TTS pipeline.
