...
**Tests:** run with `pytest` (venv). `tests/` covers the text-processing and persistence seams: emotion-tag parsing, incremental `<think>` stripping, memory recency, performance-state resume, and the age-profile schema + prompt assembly (`test_age_profiles.py`). Write `async def test_*` — asyncio auto-mode means no `@pytest.mark.asyncio` needed. No linter/formatter is configured.
...
- `src/core/age_engine.py` — maps `hours_elapsed` → one of 8 age stages, loads that stage's `profiles/age_XX_YY/personality.yaml`, and **builds the LLM system prompt** (`build_personality_prompt`) by combining permanent identity (from config) with that stage's profile.
...
- **Age-profile schema.** Keys are **Spanish** and written in **second person (voseo)** — the prompt addresses her ("Querés: …"), so the profile must too; mixing persons pushes the model into third-person narration, which the config identity explicitly forbids. Nine fields per stage: `motor_dramatico` (`quiere`/`obstaculo`/`tacticas`), `cree_de_ximena`, `estado_base`, `ritmo`, `vocabulario`, `mirada` (`nota`/`lee`/`rompe_el_silencio`), `ya_no`, `circunstancias_dadas`, `muestras` (`situacion` + `ximena` *or* `otro` + `indira`). `build_personality_prompt` emits them in a deliberate order — objective first so it colours everything after it, exemplars last. **Every field must be playable**: it has to say what she does when something happens. An abstract trait list (`rasgos`, removed) fails that test and is also where a generated profile collapses into the model's default warm-wisdom register — character here is carried by objective + tactics + `ritmo`/`vocabulario` + `muestras`, not by an attribute list. **The eight `quiere` lines are the arc**: the piece is improvised and has no plot, so they are the only place its shape lives. `muestras` must stay labelled as examples in the prompt — anything echoed verbatim goes out the speaker. Guarded by `tests/test_age_profiles.py`; the rationale for the schema is in the dramaturgy brief linked from the project memory.
